...
def plotVxlsPerAtm(pdbName     = 'untitled',
    ...
    plotHist(datax    = datax,
             plotType = plotType,
             xlabel   = '# voxels per atom',
...
def plotDensForAtm(pdbName     = 'untitled',
                   where       = '',
                   vxlsPerAtom = {},
                   plotType    = 'histogram',
                   saveFig     = True,
    ...
    if printText:
        print 'Plotting {} plots of density values per-atom (for each atom)...'.format(plotType)
    ...
    for atm in PDBarray:
        ...
        plotHist(datax    = datax,
                 ...
                 title    = 'Plot of raw densities assigned to atom: {}'.format(atm.getAtomID()),
                 outDir   = where + 'plots/rawDensitiesForAtoms/',
                 saveFig  = saveFig,
                 fName    = '{}-densForAtm-{}_{}{}'.format(pdbName,atm.getAtomID(),plotType,saveType))
