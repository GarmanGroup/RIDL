...
from metricNormalisation import metricNormalisation
from pandas import DataFrame
import string
from scipy.stats import ttest_ind, skew, kurtosis, mstats, ks_2samp
from scipy.stats import linregress, f, anderson_ksamp
import numpy as np
import operator
import os
...
class combinedAtomList(object):
    ...
    def __init__(self,
                 datasetList=[], numLigRegDsets=1, doseList=[],
                 initialPDBList=[], outputDir='./', partialDatasets=True,
                 seriesName='untitled', inclFCmetrics=False):

        self.datasetList = datasetList

        # number of datasets to perform linear regression over
        self.numLigRegDatasets = numLigRegDsets

        # list of (DWD)doses for each dataset
        self.doseList = doseList

        # list of atom objects for initial (lowest dose) pdb file
        self.initialPDBList = initialPDBList

        self.outputDir = outputDir

        # Bool, whether atoms only in subset of datasets are included
        self.partialDatasets = partialDatasets
    ...
    def getMultiDoseAtomList(self,
        ...
        if len(self.datasetList) > 1:
            self.printOrWriteToLog(
                logFile=logFile, txt='Multiple datasets detected...')
            for dataset in self.datasetList:
                if len(dataset) != len(self.datasetList[0]):
                    self.printOrWriteToLog(
                        logFile=logFile,
                        txt='Not all PDB structures have same number of' +
                            'atoms!\nWill only include atoms common ' +
                            'to ALL structures...')
                break
        elif len(self.datasetList) == 1:
            self.printOrWriteToLog(
                logFile=logFile, txt='Single dataset detected...')

        PDBdoses = []
        numNotFoundAtoms = 0

        self.printOrWriteToLog(
            logFile=logFile, txt='Locating common atoms to ALL datasets...:')
        ...
        multiDimAttrs = ['Bfactor', 'Occupancy', 'meandensity', 'maxdensity',
                         'mindensity', 'mediandensity', 'numvoxels',
                         'stddensity', 'min90tile', 'max90tile', 'min95tile',
                         'max95tile', 'meanNegOnly', 'meanPosOnly']

        if self.inclFCderivedMetrics:
            extraMetrics = ['fracOfMaxAtomDensAtMin', 'densityWeightedMean',
                            'densityWeightedMin', 'densityWeightedMax',
        ...
        for atom in self.datasetList[0]:
            ...
            for dataset in self.datasetList[1:]:
                ...
                for otheratom in dataset:
                    k += 1
                    if atom.getAtomID() == otheratom.getAtomID():
                        atm_counter += 1
            ...
            if atm_counter != len(self.datasetList) and not self.partialDatasets:
                self.printOrWriteToLog(
                    logFile=logFile,
                    txt='Atom "{}" not found in all'.format(atom.getAtomID()) +
                ...
                for attr in singDimAttrs+multiDimAttrs:
                    if attr[0] != '_':
                        if not isinstance(atomDict[attr], list):
                            ...
                            newatom.getDensMetricInfo(
                                metric=metName, normType='Standard',
                                values=atomDict[attr])

                if atm_counter != len(self.datasetList):
                    self.printOrWriteToLog(
                        logFile=logFile,
                        txt='Atom "{}" not found in'.format(atom.getAtomID()) +
                            ' all datasets\n---> including partial ' +
                            'information in atom list...')
                    numNotFoundAtoms += 1
                PDBdoses.append(newatom)

        if self.partialDatasets:
            self.printOrWriteToLog(
                logFile=logFile, txt='# atoms not in all datasets: {}'.format(
                    numNotFoundAtoms))
        else:
            self.printOrWriteToLog(
                logFile=logFile,
                txt='# atoms removed since not in all datasets: {}'.format(
                    numNotFoundAtoms))

        self.printOrWriteToLog(logFile=logFile, txt='---> Finished!')

        self.atomList = PDBdoses

    def printOrWriteToLog(self, logFile='', txt=''):
    ...
    def getAtom(self, chain='', restype='', resnum='',
                atomtype='', printOutput=False):
    ...
    def getDensMetrics(self):
        ...
        currentMetrics = []
        for metric in list(self.atomList[0].densMetric.keys()):
            for normType in list(self.atomList[0].densMetric[metric].keys()):
                currentMetrics.append([metric, normType])
        return currentMetrics

    def getFormattedmetricName(self,
                               metric='loss', norm='Standard',
                               form='HTML', normSet=[['', 'CA']]):
        ...
        if form == 'HTML':
            if norm == 'Standard':
                ...
                if normSet == [['', 'CA']]:
                    if metric == 'loss':
                        ...
                        name = r"\(C_\alpha\text{-normalised}\ D_\text{neg}\text{(atom)}\)"
                    else:
                        name = 'Calpha-normalised ' + metric
                else:
                    if metric == 'loss':
                        name = r"\text{Custom-normalised}\ D_\text{loss}\text{(atom)}\)"
                    elif metric == 'density_weighted_loss':
                        name = r"\text{Custom-normalised}\ D_\text{loss}^{\rho}\text{(atom)}\)"
                    elif metric == 'density_weighted_mean_negOnly':
                        name = r"\text{Custom-normalised}\ D_\text{neg}\text{(atom)}\)"
                    else:
                        name = 'Custom-normalised ' + metric
            else:
                name = norm + ' ' + metric
            ...
            if norm == 'Standard':
                if metric == 'loss':
                    ...
                    name = r"$D_\mathrm{neg}\mathrm{(atom)}$"
                else:
                    name = metric
            elif norm in ('X-normalised', 'Calpha normalised'):
                if normSet == [['', 'CA']]:
                    if metric == 'loss':
                        ...
                        name = r"$C_\alpha\mathrm{-normalised}\ D_\mathrm{neg}\mathrm{(atom)}$"
                    else:
                        name = 'Calpha-normalised ' + metric
                else:
                    if metric == 'loss':
                        name = r"$\mathrm{Custom-normalised}\ D_\mathrm{loss}\mathrm{(atom)}$"
                    elif metric == 'density_weighted_loss':
                        name = r"$\mathrm{Custom-normalised}\ D_\mathrm{loss}^{\rho}\mathrm{(atom)}$"
                    elif metric == 'density_weighted_mean_negOnly':
                        name = r"$\mathrm{Custom-normalised}\ D_\mathrm{neg}\mathrm{(atom)}$"
                    else:
                        name = 'Custom-normalised ' + metric
            else:
                name = norm + ' ' + metric
    ...
    def calcAdditionalMetrics(self,
        ...
        # 'normalisationSet' is only used if a metric normalisation is done

        options = ['Calpha normalised', 'X-normalised',
                   'net', 'lin reg', 'dataset 1 subtracted',
                   'average', 'vector weighted', 'vector subtracted',
                   'Standardised']
        ...
        for atom in self.atomList:
            ...
            if newMetric in 'X-normalised':
                ...
                atom.calcLinReg(
                    numLinRegDsets=self.numLigRegDatasets,
                    normType='Standard', metric=metric)

            elif newMetric == 'net':
                atom.calcNetChangeMetric(normType='Standard')

            elif newMetric == 'dataset 1 subtracted':
                atom.calcFirstDatasetSubtractedMetric(
                    normType='Standard', metric=metric)

            elif newMetric == 'average':
                atom.calcAvMetric(normType, metric)

            elif newMetric == 'vector weighted':
                atom.calcVectorWeightedMetric(
                    metric=metric, normType='Standard', vector=vector)

            elif newMetric == 'vector subtracted':
                atom.calcVectorSubtractedMetric(
                    metric=metric, normType='Standard', vector=vector)

            elif newMetric == 'Standardised':
                atom.calcStandardisedMetric(
                    metric=metric, normType='Standard',
                    meanOfDistn=meand, stdOfDistn=stdd)

    def writeMetric2File(self,
                         where='./', groupBy='none', metric='loss',
                         normType='Standard', sortby='metric', numDP=2):
        ...
        for i in range(0, metricLength):
            if groupBy == 'none':
                ...
                csvfile.write('{} (mean),{} (std dev)'.format(
                    str(i+1), str(i+1)))
            if i != metricLength-1:
                csvfile.write(',')
        csvfile.write('\n')
        ...
        if groupBy != 'none':
            ...
            statDic = {k: [] for k in list(TypeDic.keys())}

            for d in range(metricLength):
                if groupBy == 'atomtype':
                    a, b = self.getPerAtmtypeStats(
                        metric=metric, normType=normType, dataset=d, n=1)
                elif groupBy == 'residue':
                    a, b = self.getPerResidueStats(
                        metric=metric, normType=normType, dataset=d, n=1)
            ...
            if sortby == 'atomnum':
                ...
                self.atomList.sort(
                    key=lambda x: x.densMetric[metric][normType]['values'][0], reverse=True)
            for atom in self.atomList:
    ...
    def findProbAboveAvDam(self,
                           metric='loss', normType='Calpha normalised',
                           threshold=0):
    ...
    def findMetricRatio(self,
                        metric='loss', normType='Standard', resiType='',
                        atomType1='', atomType2=''):

        # find ratio between metric for two atom
        # types within a single residue type

        title = '\n{} D{} comparison between {}-{} and {}-{} atoms'.format(
            normType, metric, resiType, atomType1, resiType, atomType2)
        print('\n'+'-'*len(title)+title)
        dic, ratioDic = {}, {'distance': [], 'ratio': []}

        atms1 = self.getAtom(restype=resiType, atomtype=atomType1)
        if atms1 == []:
            return
        ...
        for atom in self.atomList:
            if atom.basetype == resiType and atom.atomtype == atomType2:
                ...
                # check that generated 'key' exists and skip if not
                # (needed for incomplete residues)
        ...
        for key in ratioDic:
            ...
            outputString += '\nmean\t' + '\t'.join(
                [str(round(val, 3)) for val in list(np.mean(ratioDic[key], 0))])
            outputString += '\nmedian\t' + '\t'.join(
                [str(round(val, 3)) for val in list(np.median(ratioDic[key], 0))])
            outputString += '\nstd dev\t' + '\t'.join(
                [str(round(val, 3)) for val in list(np.std(ratioDic[key], 0))])
            print(outputString)
    ...
    def findMetricRatioKeyResidues(self,
                                   metric='loss', normType='Standard',
                                   rType='ratio', errorBars=True,
                                   pairs=[], title='', fileType='.svg',
                                   axesFont=18, legendFont=18, titleFont=24):
        ...
        fileout = open('{}{}_{}-D{}-{}.txt'.format(
            self.outputDir, title, normType, metric, rType), 'w')
        ...
        for pair in pairs:
            ...
            output = self.findMetricRatio(
                metric=metric, normType=normType, resiType=pair[0],
                atomType1=pair[1], atomType2=pair[2])
        ...
        plt.gca().set_color_cycle([colormap(i) for i in np.linspace(0, 0.9, len(list(foundPairs.keys())))])
        for key in list(foundPairs.keys()):
            xData = self.doseList
        ...
        fig.suptitle('{} D{} {}'.format(
            normType, metric, rType), fontsize=titleFont)

        figName = '{}metric-D{}_normalisation-{}_{}_{}'.format(
            self.outputDir, metric, normType, rType, title)
        fileName = self.checkUniqueFileName(
            fileName=figName, fileType=fileType)
    ...
    def calcMetricDiffFromStructureMean(self,
        ...
        for atom in self.atomList:
            if diff == 'difference':
                ...
                atom.calcRatioToMeanMetric(
                    metric=metric, normType=normType, avMetric=av)
            elif diff == 'std-devs':
                atom.calcNumStdFromMeanMetric(
                    metric=metric, normType=normType, avMetric=av,
                    stdMetric=std)

    def numAtmsWithMetricAboveLevel(self,
                                    dataset=0, metric='loss',
                                    normType='Standard',
                                    threshold=0, printStr=False,
                                    firstTime=True, atomType='',
                                    resType=''):
        ...
        if firstTime:
            self.calcMetricDiffFromStructureMean(
                metric=metric, normType=normType, diff='std-devs')
        if atomType == '' and resType == '':
            aList = self.atomList
        else:
            aList = self.getAtom(
                restype=resType, atomtype=atomType)
        ...
        for d in dList:
            count = 0
            for atm in aList:
                if atm.densMetric[metric]['num stds']['values'][d] > threshold:
                    count += 1
            if printStr:
                print('For dataset {}:'.format(d))
                print('# atoms with {} D{} metric above {}'.format(
                    normType, metric, threshold) +
    ...
    def findMetricRatioKeyResidues_scatterplot(
        self, metric='loss', normType='Standard', rType='ratio',
        pairs=[['GLU', 'CD', 'CG'], ['GLU', 'CD', 'OE1']],
        title='', fileType='.svg', axesFont=18, legendFont=18,
            titleFont=24):
        ...
        i, j = 0, -1
        cmap = plt.cm.Set1
        nsteps = len(list(foundPairs.keys()))

        colors = ['#737474', '#409cd6', '#58bb6b',
                  '#faa71a', '#ff6a6a']
        ...
        for key in list(foundPairs.keys()):
            ...
            plt.scatter(xData, yData, marker='o', s=100, c=colors[j],
                        edgecolors='#FFFFFF', label=key)
        ...
        fileName = self.checkUniqueFileName(
            fileName=figtitle, fileType=fileType)
        fig.savefig(fileName)

        return DataSave

    def checkUniqueFileName(self,
                            fileName='', fileType='.svg'):
        ...
        fname = lambda x: strippedName.replace(
            fileType, '_{}{}'.format(x, fileType))
        i = 1
    ...
    def findTopAtomOfType(self,
                          metric='loss', normType='Standard',
                          dataset='all', atomType='OH',
                          resType='TYR', printText=True):
        ...
        for d in dList:
            if printText:
                print('For dataset: {}'.format(d))
            orderedAtms = self.getTopNAtoms(
                metric=metric, normType=normType, dataset=d, n='all')
    ...
    def getTopNAtoms(self,
        ...
        if dataset != 'all':
            atomList = self.getAtomListWithoutPartialAtoms(dataset=dataset)
            ...
            for k in list(ranks.keys()):
                overallRank[k] = np.product(list(map(float, ranks[k])))

            top = sorted(iter(overallRank.items()), key=operator.itemgetter(1), reverse=r)[:n]
            atomList = [t[0] for t in top]
            return atomList

    def getTopNAtomsPDBfile(self,
                            metric='loss', normType='Standard', dataset=0,
                            n='all',  pdbFile='untitled.pdb'):
        ...
        pdbOutName = '{}top{}-{}D{}sites-dset{}.pdb'.format(
            self.outputDir, n, normType, metric, dataset)
        ...
        for d in datasets:
            ...
            topAtms = self.getTopNAtoms(
                metric=metric, normType=normType, dataset=d, n=n)
            count = 0
            for atm in topAtms:
                count += 1
                # generate pdb line - note: absolute value of metric
                # taken since Bfactor>0 (for displaying in pymol etc.)
                l = writePDBline_DamSite(
                    atom=atm, index=count, chain=chains[d],
    ...
    def getTopNAtomsString(self,
                           metric='loss', normType='Standard',
                           incOtherMetrics=True, dataset=0,
                           useInHTML=True, splitBy=',', n=25):
        ...
        topN = self.getTopNAtoms(
            metric=metric, normType=normType, dataset=dataset, n=n)
        atomInfoList = []
        for atom in topN:
            ...
            if metric == 'loss':
                if self.inclFCderivedMetrics:
                    # this gives an indication of how far away the
                    # Dloss-valued voxel is from the atomic centre
            ...
            if incOtherMetrics:
                ...
                if metric == 'loss':
                    metricsToInclude = ['mean', 'gain', 'Bfactor']
        ...
        if metric == 'loss':
            stringOut = 'Atom-ID{}Dloss{}Dloss ({}-normalised){}'.format(
                splitBy, splitBy, calphaStr, splitBy)
            if self.inclFCderivedMetrics:
            ...
            stringOut = 'Atom-ID{}DW-mean (-ve only){}DW-mean (-ve only) ({}-normalised){}'.format(
                splitBy, splitBy, calphaStr, splitBy)
    ...
    def getTopNAtomsDotPlot(self,
                            ...
                            dataset=0, numHits=25, savePlot=True, fileType='.png'):
        ...
        primaryMetric = self.getFormattedmetricName(
            metrics[0], normTypes[0], form='TEX')

        topN = self.getTopNAtoms(
            metric=metrics[0], normType=normTypes[0],
            dataset=dataset, n=numHits)
        ...
        # Make the PairGrid
        g = sns.PairGrid(
            plotdf.sort_values(primaryMetric, ascending=False),
            x_vars=metricsToPlot, y_vars=['Atom ID'],
            size=14, aspect=.25)

        # Draw a dot plot using the stripplot function
        g.map(sns.stripplot, size=20, orient="h", edgecolor="gray",
              palette=sns.color_palette("Blues_r", numHits))
        g.set(ylabel="")

        titles = metricsToPlot
        ...
        if savePlot:
            ...
            saveName = '{}plots/{}dotPlot_top-{}-DamSites_dset-{}{}'.format(
                self.outputDir, plotDir, numHits, dataset, fileType)
            saveName = self.checkUniqueFileName(
                fileName=saveName, fileType=fileType)
            g.savefig(saveName)

        return saveName

    def plotAtomtypeRankingWithDataset(
        self, metric='loss', normType='Standard',
        residue=['TYR', 'TYR', 'TYR', 'TYR', 'GLU', 'GLU', 'GLU', 'GLU'],
        atomtype=['OH', 'CZ', 'CE1', 'CE2', 'CG', 'CD', 'OE1', 'OE2'],
        figTitle='', axesFont=18, legFont=12, titleFont=20,
            saveFig=True, percent=True, outputDir='', fileType='.svg'):
        ...
        for res, atm in zip(residue, atomtype):
            ...
            rankList[aTag] = []
            for d in self.getDsetList():
                rank = self.getAtomtypeRanking(
                    metric=metric, normType=normType, dataset=d,
                    residue=[res], atomtype=[atm])
                if percent:
                    rankList[aTag].append(rank[aTag]['Ratio'])
                else:
                    rankList[aTag].append(rank[aTag]['Rank'])

        args = [metric, normType, residue, atomtype]
        ...
        fileName = self.makeLinePlot(
            vals2Plot=rankList, xVals='doses', yLabel=ylabel,
            axesFont=axesFont, legFont=legFont, titleFont=titleFont,
            title=title, fileType=fileType, saveFig=saveFig,
            fileName=name, outputDir=outputDir)
    ...
    def getAtomtypeRanking(self,
                           metric='loss', normType='Standard',
                           dataset=0, residue=['TYR'],
                           atomtype=['OH'], printText=False):
        ...
        for d in dList:
            ...
            sortedVals = sorted(list(atomVals.items()), key=operator.itemgetter(1))
        ...
        atomRanksOverall = {}
        for key in list(atomRanks.keys()):
            atomRanksOverall[key] = np.product(atomRanks[key])

        sortedRanks = sorted(list(atomRanksOverall.items()), key=operator.itemgetter(1))
        ...
        for res, atm in zip(residue, atomtype):
            ...
            ranks[keyAtm] = {'Rank': keyAtmPos, 'Ratio': keyAtmRank}

        return ranks

    def breakdownTopNatomsBy(self,
                             metric='loss', normType='Standard',
                             dataset=0, n=0, sortby=['basetype', 'atomtype']):

        # find the top n atoms and sort them by grouping 'sortby'

        topN = self.getTopNAtoms(
            metric=metric, normType=normType,
            dataset=dataset, n=n)
        countDic, totalDic = {}, {}
        for atom in topN:
            vals = [getattr(atom, att) for att in sortby]
            atomID = '-'.join(vals)
            if atomID not in list(countDic.keys()):
                countDic[atomID] = 1
                totalDic[atomID] = self.getNumAtomsOfType(
                    atts=sortby, vals=vals)
            else:
                countDic[atomID] += 1
        headerString = 'Partition of top atom {} D{} values by {}.\n'.format(
            normType, metric, ' & '.join(sortby))
        # n as fraction of total atoms
        fracOfTotal = round((float(n)/self.getNumAtoms())*100, 2)
        headerString += 'For dataset {}, top {} ({}%) atoms included\n'.format(
            dataset, int(n), fracOfTotal)
    ...
    def getNumAtomsOfType(self,
        ...
        i = 0
        for atom in self.atomList:
            if vals == [getattr(atom, att) for att in atts]:
                i += 1
        return i

    def getTopAtomsStackedBarplot(self,
                                  metric='loss', normType='Standard',
                                  n=25, palette='hls', barWidth=0.5,
                                  axesFont=18, titleFont=18, legendFont=14,
                                  saveFig=True, outputDir='./',
                                  fileType='.svg', plotTitle=''):
        ...
        for d in range(numDsets):
            stats = self.breakdownTopNatomsBy(
                metric=metric, normType=normType, dataset=d,
                n=n, sortby=['basetype'])
            ...
            for key in list(breakdown.keys()):
                if key not in list(plotDic.keys()):
                    plotDic[key] = [0]*d + [breakdown[key]]
                else:
                    plotDic[key] += [breakdown[key]]
            for key in list(plotDic.keys()):
        ...
        numResis = len(list(plotDic.keys()))
        colors = sns.color_palette(palette, n_colors=numResis, desat=0.9)
        for i, (k, color) in enumerate(zip(list(plotDic.keys()), colors)):

            fd = DataFrame()
            fd['Dataset'] = np.array(list(range(numDsets)))+1
            fd['Frequency'] = accumulatedVals
            sns.barplot(x='Dataset', y='Frequency',
                        data=fd, label=k, color=color)
        ...
        if not saveFig:
            ...
            args = [outputDir, n, metric, normType.replace(" ", "")]
    ...
    def getPerAtmtypeStats(self,
                           metric='loss', normType='Standard',
                           dataset=0, sortby='mean', n=25):

        # for a given metric type, determine per-atom-type
        # statistics on the distribution of damage

        atmtypeDict = self.groupByAtmType(dataset=dataset)

        statsDic = self.getStats(metric=metric, normType=normType,
                                 dataset=dataset, dic=atmtypeDict)

        statsString = self.reportStats(
            stats=statsDic, name='Type', sortby=sortby,
            n=n, normType=normType)
    ...
    def getPerResidueStats(self,
        ...
        # group atoms by residue type
        resDict = self.groupByResType(dataset=dataset)

        statsDic = self.getStats(
            metric=metric, normType=normType,  dataset=dataset, dic=resDict)

        statsString = self.reportStats(
            stats=statsDic, name='Residue', sortby=sortby,
            n=n, normType=normType)
    ...
    def getPerChainStats(self,
        ...
        # group atoms by chain type
        chainDict = self.groupByChainType(dataset=dataset)

        statsDic = self.getStats(
            metric=metric, normType=normType, dataset=dataset, dic=chainDict)

        statsString = self.reportStats(
            stats=statsDic, name='Chain', sortby=sortby,
            n=n, normType=normType)
    ...
    def getStructureStats(self,
        ...
        statsDic = self.getStats(
            metric=metric, normType=normType,
            dataset=dataset, dic={'Structure': self.atomList})

        statsString = self.reportStats(
            stats=statsDic, name='Structure', normType=normType, n=1)
    ...
    def getStats(self,
        ...
        for k in list(dic.keys()):
            metricList = [atom.densMetric[metric][normType]['values'][dataset] for atom in dic[k]]
            statsDic = {}
            m = metricList
            ...
            # statsDic['zero-asym1'] = self.calcAsymmetryAboutZero(vals=m, method=1)
            statsDic['asymmetry score'] = self.calcAsymmetryAboutZero(vals=m)

            statsDic['outliers'] = self.calcNumOutliers(
                metricList=m, metric=metric, normType=normType)
            statsDic['normality'] = self.testForNormality(metricList=m)

            statsDic['returnOrder'] = ['mean', 'std', '#atoms', 'outliers',
                                       'skew', 'kurtosis', 'asymmetry score']

            getStatsPerKey[k] = statsDic

        return getStatsPerKey

    def reportStats(self,
                    stats={}, name='Residue', sortby='mean',
                    n=20, numDp=2, normType='Standard', format='txt'):
        ...
        sortbyDict = stats[list(stats.keys())[0]]
        headerOrder = sortbyDict['returnOrder']
        headerString = name+'\t\t'
        headerString += '\t\t'.join(headerOrder) + '\n'
        if sortby not in list(sortbyDict.keys()):
            return 'Unexpected ranking specified'
        if n != 'all' and not isinstance(n, int):
            sys.exit('input "n" must be integer or "all", unable to complete')
        ...
        for key in list(stats.keys()):
            ...
            for key2 in headerOrder:
                if isinstance(statsDic[key2], float):
                    statsFmtd.append('{}'.format(round(statsDic[key2], numDp)))
                else:
                    statsFmtd.append(str(statsDic[key2]))
            string = '\t\t'.join([str(key)]+statsFmtd)
            list1.append(string)
            list2.append(statsDic[sortby])

        # sort by chosen 'sortby' parameter
        sortedList1 = [x for (y, x) in sorted(zip(list2, list1))]
        sortedList1.reverse()
    ...
    def getStatPerDataset(self,
                          metric='loss', normType='Standard',
                          stat=['mean', 'skew'], inclPartialAtms=True):
        ...
        for i in self.getDsetList():
            stats = self.getPerResidueStats(
                metric=metric, normType=normType, dataset=i, n='all')
            if i == 0:
                for k in list(stats[1].keys()):
                    for s in stat:
        ...
        if not inclPartialAtms:
            ...
            for s in list(statPerDset.keys()):
                for k in list(statPerDset[s].keys()):
    ...
    def plotStatVsDataset(self,
                          ...
                          saveFig=True, axesFont=18, titleFont=16,
                          legFont=10, figTitle='', outputDir='./',
                          fileType='.svg', palette='hls', printTxt=False):
        ...
        statPerDset = self.getStatPerDataset(
            stat=stat, metric=metric, normType=normType,
        ...
        for s in stat:
            ...
            fileName = self.makeLinePlot(
                palette=palette, vals2Plot=statPerDset[s],
                xVals='doses', yLabel=s, axesFont=axesFont,
                legFont=legFont, titleFont=titleFont, title=title,
                fileType=fileType, saveFig=saveFig, fileName=name,
                outputDir=outputDir)
    ...
    def plotStatVsStat(self,
                       dataset='all', metric='loss', normType='Standard',
                       stat1=['mean', 'mean', 'mean', 'skew'],
                       stat2=['skew', 'kurtosis', 'std', 'kurtosis'],
                       saveFig=True, axesFont=18, titleFont=20,
                       legFont=14, figTitle='', outputDir='./',
                       fileType='.svg', palette='husl', markSize=40,
                       printTxt=False):
        ...
        statPerDset = self.getStatPerDataset(
            stat=stats, metric=metric, normType=normType,
            inclPartialAtms=False)
        ...
        for (s1, s2) in zip(stat1, stat2):
            ...
            f, axes = plt.subplots(num_cols, num_rows+1,
                                   sharex=True, sharey=True)

            col, row = -1, 0
            uniqPoints, uniqLabels = [], []
            for d in self.getDsetList():
                col += 1
                if col >= num_cols:
                    col = 0
                    row += 1
                numResis = len(list(statPerDset[s1].keys()))

                for (k, color) in zip(list(statPerDset[s1].keys()), sns.color_palette(palette, numResis)):
                    p = axes[col, row].scatter(
                        statPerDset[s1][k][d], statPerDset[s2][k][d],
                        label=k, c=color, s=markSize)
                    if col == 0 and row == 0:
                        uniqPoints.append(p)
                        uniqLabels.append(k)
                sns.despine()

            f.text(0.5, 0.04, s1, ha='center', fontsize=axesFont)

            f.text(0.04, 0.5, s2, va='center',
                   rotation='vertical', fontsize=axesFont)

            # place legend outside to right of plot
            box = axes[col, row].get_position()
            axes[num_cols-1, num_rows//2].legend(
                loc='top right', bbox_to_anchor=(1.5, 1.3), fontsize=legFont)

            args = [metric, normType, s1, s2]
            ...
            f.suptitle(title, fontsize=titleFont)
            ...
            if not saveFig:
                ...
                fileName = self.checkUniqueFileName(
                    fileName=outputDir+name, fileType=fileType)
                f.savefig(fileName)
                fileNames.append(fileName)
        if saveFig:
            return fileNames
    ...
    def twoAtomTypeTtest(self,
                         metric='loss', normType='Standard', dataset=0,
                         atomType1=['OH'], atomType2=['OE1', 'OE2', 'OD1', 'OD2'],
                         resType1=['TYR'], resType2=['GLU', 'GLU', 'ASP', 'ASP'],
                         alpha=0.01):
        ...
        for atm, res in zip(atomType1, resType1):
            atmList1 += self.getAtom(restype=res, atomtype=atm)
        atmList2 = []
        for atm, res in zip(atomType2, resType2):
            atmList2 += self.getAtom(restype=res, atomtype=atm)
    ...
    def calcAsymmetryAboutZero(self,
                               vals=[], threshold=0, method=2):

        # calculate the magnitude of asymmetry of a
    ...
    def calculateSkew(self,
        ...
        skewVal = skew(metricList, axis=0, bias=True)
    ...
    def calculateKurtosis(self,
        ...
        kurt = kurtosis(metricList, axis=0, bias=True)

        return kurt

    def testForNormality(self,
                         metricList=[], suppressText=True):
    ...
    def groupByAtmType(self,
        ...
        for atom in self.atomList:
            ...
            if atmtype not in list(atmtypeDict.keys()):
    ...
    def groupByResType(self,
        ...
        for atom in self.atomList:
            if dataset not in atom.getPresentDatasets():
                continue
            if atom.basetype not in list(resDict.keys()):
    ...
    def groupByChainType(self,
        ...
        for atom in self.atomList:
            if dataset not in atom.getPresentDatasets():
                continue
            if atom.chaintype not in list(chainDict.keys()):
                chainDict[atom.chaintype] = [atom]
            else:
                chainDict[atom.chaintype].append(atom)
        return chainDict

    def checkSpecificAtomsExist(self,
                                AtomSet=[['GLU', 'CA']]):

        # check that specified atoms
        # actually exist in structure

        count = 0
        for a in AtomSet:
            atom = self.getAtom(
                restype=a[0], atomtype=a[1], printOutput=False)
            if len(atom) > 0:
                count += 1
        if count == 0:
            return False
    ...
    def checkCalphaAtomsExist(self,
        ...
        for res in self.aminoAcids:
            atom = self.getAtom(
                restype=res, atomtype='CA', printOutput=False)
            if len(atom) == 0:
                count += 1
    ...
    def densityMetricHeatMap(self,
                             metric='loss', normType='Standard',
                             saveFig=False, fileType='.svg',
                             titleFont=24, outputDir='./',
                             forceSquares=True, columnPerChain=False):
        ...
        if columnPerChain:
            ...
            for atom in self.atomList:
                ...
                cj = c + str(j)
                if cj not in list(data.keys()):

                    data[cj] = {'Atom': [], 'Dataset': [], 'metric': []}
                    newChains.append(cj)

                data[cj]['metric'] += atom.densMetric[metric][normType]['values']
                data[cj]['Atom'] += [atom.getAtomID()]*self.getNumDatasets()
                data[cj]['Dataset'] += list(range(1, self.getNumDatasets() + 1))
            chains = newChains

        numChains = len(list(data.keys()))

        sns.set_context("notebook")
        chainLengths = np.array([len(data[cn]['Atom']) for cn in chains])//self.getNumDatasets()
        # get largest chain length
        maxLength = int(2.0*max(chainLengths))
        ...
        gs = GridSpec(maxLength+1, numChains+1,
        ...
        if metric == 'loss' and normType == 'Standard':
            ...
            pal = sns.diverging_palette(
                220, 10, sep=80, n=7, as_cmap=True)

        # find overall max and min metric values
        vals = [data[k]['metric'] for k in list(data.keys())]
        ...
        for i, c in enumerate(chains):
            ...
            d = d.pivot("Atom", "Dataset", "metric")
            
            f = sns.heatmap(
                d, ax=ax[i+1], cbar=i == 0,
                cbar_ax=None if i else ax[0],
                cmap=pal, vmin=lowVal, vmax=highVal,
                square=forceSquares, linewidths=1,
                linecolor='white', cbar_kws={"orientation": "vertical"})
        ...
        figTitle = plt.suptitle(
            'Metric: $D_{' + str(metric) + '}$'+'\nNormalisation: {}'.format(normType),
        ...
        if not saveFig:
            ...
            saveName = '{}heatmap_metric-{}_normalisation-{}'.format(
                outputDir, metric, normType)
            fileName = self.checkUniqueFileName(
                fileName=saveName, fileType=fileType)
            fig.savefig(fileName, bbox_extra_artists=(figTitle,),
                        bbox_inches="tight")
    ...
    def detectSuspiciousAtoms(self,
                              dataset=0, metric='loss', normType='Standard',
                              threshold=5, suppressText=True):
    ...
    def getDsetList(self):
        ...
        return list(range(self.getNumDatasets()))
    ...
    def graphMetricDistn(self,
                         metric='loss', normType='Standard', valType='average',
                         plotType='both', resiType='all', resSplit=False,
                         sideOnly=False, requireAll=False, save=True,
                         xLabel='', fileType='.svg', outputDir='./',
                         printText=True, axesFont=18, legFont=14,
                         titleFont=20, plotTitle='', inclTitle=True,
                         calcKSstat=False, calcADstat=False):
        ...
        if valType == 'all':
            ...
            for (i, color) in zip(self.getDsetList(), sns.color_palette('rainbow', numDsets)):
                presentAtms = self.getAtomListWithoutPartialAtoms(
                    dataset=i, atmList=atmList)

                if resiType == 'all':
                    data = [atm.densMetric[metric][normType]['values'][i] for atm in presentAtms]
                    lbl = 'Dataset {}'.format(i+1)
                    datax[lbl] = {'values': data, 'colors': color}
            ...
            self.calcAverageMetricOverDoses(metric=metric, normType=normType)
            ...
            if resiType == 'all':
                ...
                if valType == 'average':
                    ...
                    datax['average'] = {'values': data, 'colors': 'r'}

                else:
                    presentAtms = self.getAtomListWithoutPartialAtoms(
                        dataset=valType, atmList=atmList)
                    ...
                    if resSplit:
                        ...
                        for atm in presentAtms:
                            ...
                            try:
                                ...
                                datax[lblFull] = {'values': [vals], 'colors': ''}
                        ...
                        datax[lbl] = {'values': data, 'colors': 'r'}
                        dataxOrder.append(lbl)
            else:
                ...
                # calculate Kolmogrov-Smirnov statistic in case where
                # strictly 2 residue types plotted
                vals = [datax[k]['values'] for k in list(datax.keys())]

                if calcKSstat and len(list(datax.keys())) == 2:
                    (KSstat, KSpval) = ks_2samp(*vals)
                    stats['KS'] = {'statistic': KSstat, 'p-value': KSpval}

                if calcADstat and len(list(datax.keys())) > 1:
                    try:
                        (ADstat, critVals, ADpval) = anderson_ksamp(vals)
                        stats['AD'] = {'statistic': ADstat, 'p-value': ADpval}
                    except OverflowError:
                        pass

        if len(list(datax.keys())) == 0:
            return {}, ''
    ...
    def plotHist(self,
        ...
        if order == []:
            order = list(datax.keys())

        for k in order:
            if plotType == 'hist':
                plt.hist(
                    datax[k]['values'], nBins, histtype="stepfilled",
                    alpha=.7, label=k, color=datax[k]['colors'])

            elif plotType == 'kde':
                sns.kdeplot(
                    np.array(datax[k]['values']), shade=True,
                    label=k, color=datax[k]['colors'])

            elif plotType == 'both':
                sns.distplot(
                    np.array(datax[k]['values']),
                    label=k, color=datax[k]['colors'])

    def getKSstatBetween2Residues(self,
                                  resType1='Glu', resType2='Gln',
                                  sideOnly=False, metric='loss',
                                  normType='Standard'):
    ...
    def plotKSstatVsDataset(self,
                            metric='loss', normType='Standard', residues='all',
                            reference='GLY', sideOnly=False, inclLine=True,
                            plotType='bar', saveFig=True, axesFont=18,
                            titleFont=16, legFont=10, figTitle='',
                            saveName='', outputDir='./', fileType='.svg',
                            palette='hls'):
        ...
        for res, ref in zip(resList, reference):
            if res != reference:
                KSperDset[res+' relative to '+ref] = self.getKSstatBetween2Residues(
                    resType1=res, resType2=ref, sideOnly=sideOnly,
                    metric=metric, normType=normType)
        args = [plotType,
                metric,
                normType]
        ...
        if plotType == 'line':
            fileName = self.makeLinePlot(
                palette=palette, vals2Plot=KSperDset, inclLine=inclLine,
                xVals='doses', yLabel='Kolmogrov-Smirnov stat',
                axesFont=axesFont, legFont=legFont, titleFont=titleFont,
                title=title, fileType=fileType, saveFig=saveFig,
                fileName=name, outputDir=outputDir)

        elif plotType in ('bar', 'box'):
            pltDic = {'comparison': [], 'dataset': []}
            ...
            fileName = self.makeBarPlot(
                plotData=dFrame, xName="comparison", yName="dataset",
                yLabel='Kolmogrov-Smirnov stat', plotType=plotType,
                figName=name, saveFig=saveFig, outputDir=outputDir)

        if saveFig:
            return fileName

    def makeLinePlot(self,
                     figSize=[10, 8], palette='hls', vals2Plot={},
                     xVals='doses', errors2Plot={}, inclErrors=False,
                     inclLine=True, yLabel='', axesFont=18,
                     legFont=10, titleFont=16, title='', fileType='.svg',
                     saveFig=True, fileName='untitled', outputDir='',
                     fullDespine=False):
        ...
        if palette != 'hls':
            colors = sns.color_palette(
                palette=palette, n_colors=len(vals2Plot), desat=.9)
        else:
            colors = sns.hls_palette(
                n_colors=len(vals2Plot), l=.4, s=.8)
        ...
        if xVals == 'doses':
            ...
            x = list(xVals.values())[0]
            xLabel = list(xVals.keys())[0]

        numColors = len(list(vals2Plot.keys()))
        for i, k in enumerate(vals2Plot.keys()):
            ...
            if not inclErrors:
                ...
                if isinstance(vals2Plot[k], list):
                    plt.plot(
                        x, vals2Plot[k], label=k, linestyle=lStyle,
                        marker='o', markersize=4, color=col)
                else:
                    for i, key in enumerate(vals2Plot[k].keys()):
                        if i == 0:
                            plt.plot(
                                x, vals2Plot[k][key], label=k,
                                linestyle=lStyle, marker='o',
                                markersize=4, color=col)
                        else:
                            plt.plot(
                                x, vals2Plot[k][key], linestyle=lStyle,
                                marker='o', markersize=4, color=col)
            else:
                plt.errorbar(
                    x, vals2Plot[k], yerr=errors2Plot[k], fmt='-o',
                    capthick=2, label=k, color=col)
        ...
        box = ax.get_position()
        ax.set_position([box.x0, box.y0, box.width * 0.8, box.height])
        ax.legend(loc='center left', bbox_to_anchor=(1, 0.5), fontsize=legFont)
        plt.xlabel(xLabel, fontsize=axesFont)
        plt.ylabel(yLabel, fontsize=axesFont)
        f.suptitle(title, fontsize=titleFont)

        fileName = self.savePlot(
            outputDir=outputDir, name=fileName,
            fileType=fileType, save=saveFig, f=f)

        if saveFig:
            return fileName

    def savePlot(self,
                 outputDir='', name='untitled',
                 fileType='.svg', save=True, f=[]):
    ...
    def graphMetric(self,
                    densMet='loss', normType='Standard', atomType='',
                    resType='', resiNum='', chainType='', errorBars='NONE',
                    yLabel='', saveFig=False, outputDir='./', fileType='.svg',
                    axesFont=18, legFont=14, titleFont=20, figTitle='',
                    saveName='', figSize=[14, 8], useDoses=True, palette='hls'):
        ...
        if not isinstance(atomType, list):
            foundAtoms = self.getAtom(
                restype=resType, resnum=resiNum,
                atomtype=atomType, chain=chainType)
        else:
            errorBars = 'NONE'
            if (not isinstance(resType, list) or
                not isinstance(chainType, list) or
                    not isinstance(resiNum, list)):
            ...
            l = len(atomType)
            if (len(resType) != l or len(chainType) != l or len(resiNum) != l):
            ...
            foundAtoms = []
            for i in range(l):
                foundAtom = self.getAtom(
                    restype=resType[i], resnum=resiNum[i],
                    chain=chainType[i], atomtype=atomType[i])
        ...
        if errorBars == 'NONE':
            ...
            for atom in foundAtoms:
                ...
                if not self.checkMetricPresent(atom=atom, metric=densMet, normType=normType):
                    return
            ...
            for atom in foundAtoms:
                ...
                if getattr(atom, errorAttr) not in list(yDict.keys()):
        ...
        args = [densMet, normType, resType, resiNum, atomType]
        ...
        fileName = self.makeLinePlot(
            palette=palette, vals2Plot=yValues, xVals=xVals,
            errors2Plot=yErrors, inclErrors=inclErrors, yLabel=yLabel,
            axesFont=axesFont, legFont=legFont, titleFont=titleFont,
            title=title, fileType=fileType, saveFig=saveFig,
            fileName=name, figSize=figSize, outputDir=outputDir)

        if saveFig:
            return fileName

    def plotSusceptibleAtoms(self,
                             densMet='loss', normType='Standard',
                             errorbars=True, saveFig=True, susAtms=[],
                             fileType='.svg', axesFont=18, titleFont=24,
                             outputDir='./'):
        ...
        for key in list(sDic.keys()):
            ...
            if not errorbars:
                ...
                plt.errorbar(x, yData, yerr=yErr, fmt='-o', capthick=2,
                             label=key, color=cmap(i/float(nsteps)))

        # get structure-wide average and std dev
        av, std = self.getAverageMetricVals(densMet, normType)
        plt.errorbar(x, av, yerr=std, fmt=':o',
                     capthick=2, color='k')
    ...
    def plotNumAtomsWithMetricAboveStructureWideMean(
        self, startThreshold=0.5, thresholdDiv=0.1, metric='loss',
        normType='Standard', dataset=0, atomType='', resType='',
        axesFont=18, saveFig=True, fileType='.svg', outputLoc='./',
            titleFont=18, markerType='o', markerColor='#4872ae'):
        ...
        for d, colour in zip(dList, colours):
            ...
            n = self.numAtmsWithMetricAboveLevel(
                dataset=d, metric=metric, normType=normType,
                threshold=startThreshold, firstTime=True,
                atomType=atomType, resType=resType)
            numAtomsFound = [n]
            ...
            while n != 0:
                ...
                n = self.numAtmsWithMetricAboveLevel(
                    dataset=d, metric=metric, normType=normType,
                    threshold=t, firstTime=False, atomType=atomType,
                    resType=resType)
                numAtomsFound.append(n)
                numStds.append(t)

            plt.plot(numStds, numAtomsFound, markerType,
                     markerfacecolor='w', markeredgecolor=colour,
                     markeredgewidth=2, label='Dataset {}'.format(d))
    ...
    def checkMetricPresent(self,
                           atom='', metric='loss', normType='Standard'):
    ...
    def calcHalfDose(self,
                     chain='', restype='', resnum='', atomtype='',
                     densMet='loss', normType='Standard', shift=True,
                     offset=True):
        ...
        atom = self.getAtom(
            chain=chain, restype=restype,
            resnum=resnum, atomtype=atomtype)
        ...
        halfDoseApprox(
            atom=atom, atoms=self.atomList, doses=self.doseList,
            densityMetric=densMet, normType=normType,  plotDir=self.outputDir,
            shiftedHalfDose=shift, zeroOffset=offset)

    def calcHalfDoseForAtomtype(self,
                                restype='', atomtype='', densMet='loss',
                                normType='Standard', shift=True, offset=True,
                                n=1, fileType='.svg'):
        ...
        for atom in atoms:
            self.calcHalfDose(chain=atom.chaintype, restype=restype,
                              resnum=atom.residuenum, atomtype=atomtype,
                              densMet=densMet, normType=normType,
        ...
        self.plotScatterPlot(
            xData=xData, yData=yData, xLabel='Initial density',
            ylabel='Half-dose', figtitle='D{}: {}-{}'.format(
                densMet, restype, atomtype),
            fileType=fileType, saveTitle='D{}Scatter_{}-{}_thres{}'.format(
                densMet, restype, atomtype, n))

    def compareAtomHalfDoses(self,
                             restype='', atomtype1='', atomtype2='',
                             densMet='loss', normType='Standard',
                             n=1, fileType='.svg'):
        ...
        self.plotScatterPlot(
            xData=xData, yData=yData, fileType=fileType,
            ...
            saveTitle='D{}_halfDose{}_{}-{}'.format(
                densMet, restype, atomtype1, atomtype2))

    def HalfDoseDistn(self,
                      metric='loss', normType='Standard', plotType='kde',
                      restype='', atomtype='', save=False, nBins=300,
                      colour='b', fileType='.svg'):
        ...
        datax = {'': [atm.densMetric[metric][normType]['Half-dose'][
            'Half-dose'] for atm in atoms if atm.densMetric[metric][normType][
            'Half-dose']['Half-dose'] < 100]}

        self.plotHist(plotType=plotType, nBins=nBins,
                      datax=datax, colour={'': colour})
        ...
        if not save:
            ...
            figName = '{}halfDoseDistn-{}{}'.format(
                self.outputDir, restype, atomtype)
            fileName = self.checkUniqueFileName(
                fileName=figName, fileType=fileType)
            fig.savefig(fileName)
    ...
    def compareSensAtoms(self,
        ...
        numPairsDic = {}
        dataDic = {}
        sensAtoms = [['GLU', 'CD', 'CG'], ['GLU', 'CD', 'OE1'],
                     ['ASP', 'CG', 'CB'], ['ASP', 'CG', 'OD1'],
                     ['TYR', 'OH', 'CZ']]

        for i in sensAtoms:
            rSquared, numPairs, data = self.compareMetricsBetweenAtoms(
                restype=i[0], atomtype1=i[1], atomtype2=i[2], densMet=densMet,
                normType=normType, dataset=dataset, outputDir=outputDir)
            if rSquared is not False:
                rSquaredDic['-'.join(i)] = rSquared
                numPairsDic['-'.join(i)] = numPairs
                dataDic['_'.join(i)] = data
        return rSquaredDic, numPairsDic, dataDic

    def compareSensAtoms_RsquaredLinePlotWithDataset(
        self, densMet='loss', normType='Standard', outputDir='./',
            saveFig=True, fileType='.svg', plotType='line'):
        ...
        yValDic = {}
        for d in self.getDsetList():
            out = self.compareSensAtoms(
                densMet=densMet, normType=normType,
                dataset=d, outputDir=outputDir)

            RsquaredDic = out[0]
            for k in list(RsquaredDic.keys()):
                if k not in list(yValDic.keys()):
        ...
        name = '{}plot_Metric-D{}_Normalisation-{}_RsquaredValues'.format(
            plotType, densMet, normType)

        if plotType == 'line':
            fileName = self.makeLinePlot(
                vals2Plot=yValDic, xVals='doses', yLabel='Rsquared',
                title=title, fileType=fileType, saveFig=saveFig,
                fileName=name, outputDir=outputDir)
                ...
                fileName = self.makeBarPlot(
                    plotData=dFrame, xName="comparison", yName="dataset",
                    yLabel='R-squared stat', plotType=plotType,
                    figName=name, saveFig=saveFig, outputDir=outputDir)
    ...
    def compareMetricsBetweenAtoms(self,
        ...
        if dataset == 'av':
            self.calcAverageMetricOverDoses(metric=densMet, normType=normType)
        ...
        # plot this data as scatter plot
        figTitle = 'Metric: {}, Normalisation: {}\n for {} atoms, Dataset: {}'.format(
            densMet, normType, restype, dataset)

        saveTitle = 'scatterplot_metric-{}_normalisation-{}_{}-{}-{}_Dset-{}'.format(
            densMet, normType.replace(' ', ''), restype, atomtype1, atomtype2, dataset)

        rSquared = self.plotScatterPlot(
            xData=xData, yData=yData,
            xLabel='{}-{} D{}'.format(restype, atomtype1, densMet),
            ylabel='{}-{} D{}'.format(restype, atomtype2, densMet),
            figtitle=figTitle, fileType=fileType, saveTitle=saveTitle,
            lineBestFit=True, outputDir=outputDir)
    ...
    def compareMetrics(self,
                       restype='', atomtype='', metric1='loss', metric2='loss',
                       normType1='Standard', normType2='Standard', dSet=0,
                       fileType='.svg', outputDir='./'):
        ...
        rSquared = self.plotScatterPlot(
            ...
            fileType=fileType, lineBestFit=True, outputDir=outputDir,
    ...
    def plotScatterPlot(self,
                        xData=[], yData=[], xLabel='', ylabel='', figtitle='',
                        saveTitle='untitled', fileType='.svg', outputDir='',
                        lineBestFit=False, yequalsxLine=False,
                        colors='#50527A', axesFont=18, titleFont=24):
        ...
        plt.scatter(xData, yData, marker='o', s=100,
                    c=colors, edgecolors='#FFFFFF', cmap='copper')
    ...
    def damageVdistPlot(self,
                        type1='', type2='', dataset=0, densMet='loss',
                        normType='Standard', fileType='.svg'):
        ...
        atoms1, atoms2 = [], []
        for ind in type1:
            atoms1 += self.getAtom(restype=ind[0], atomtype=ind[1])
        for ind in type2:
            atoms2 += self.getAtom(restype=ind[0], atomtype=ind[1])
    ...
    def getMinDistToAtomType(self,
                             atomType1='OH', resType1='TYR', resNum1='330',
                             chainType1='M', atomType2='SG', resType2='CYS'):

        # get min distance from specified atom 1
        # to atom of type resType2 & atomType2

        atm1 = self.getAtom(
            chain=chainType1, restype=resType1,
            resnum=resNum1, atomtype=atomType1)

        atms2 = self.getAtom(restype=resType2, atomtype=atomType2)

        minAtm = []
        minDist = 1e6
        for atm2 in atms2:
            dist = self.getDistanceBetweenAtoms(atom1=atm1[0], atom2=atm2)
            try:
                minDist
            except NameError:
                minDist = dist
    ...
    def scatterAtmsByDistToOtherAtms(
        self, atomType1='OH', resType1='TYR', atomType2='CB', resType2='CYS',
            metric='loss', normType='Standard', outputDir='./'):

        # rank a particular type of atom (e.g. TYR-OH)
        # by shortest distance to another atom type
        # (e.g. CYS-SG) and make scatter plot

        atms1 = self.getAtom(restype=resType1, atomtype=atomType1)

        xVals = []
        yVals = []
        for atm1 in atms1:
            info = self.getMinDistToAtomType(
                atomType1=atm1.atomtype, resType1=atm1.basetype,
                resNum1=atm1.residuenum, chainType1=atm1.chaintype,
                atomType2=atomType2, resType2=resType2)
            xVals.append(info[1])
            yVals.append(atm1.densMetric[metric][normType]['values'])

        for d in self.getDsetList():
            Rsquared = self.plotScatterPlot(
                xData=xVals, yData=[y[d] for y in yVals], lineBestFit=True,
                xLabel='Distance to {}-{}'.format(resType2, atomType2),
                ylabel='{} D{}'.format(normType, metric),
                figtitle='Min {}-{} distance vs {}-{} D{}'.format(
                    resType2, atomType2, resType1, atomType1, metric),
                saveTitle='metric-{}_normalisation-{}_scatter_{}-{}_minDistTo_{}-{}_Dset-{}'.format(
                    metric, normType, resType1, atomType1, resType2, atomType2, d),
                outputDir =outputDir)

            print('Dataset {} --> R^2 = {}'.format(d+1, round(Rsquared, 4)))

    def getAtomsWithinDist(self,
                           atom='', distLimMax=4, distLimMin=0,
                           printText=False, ignoreSame='res'):
        ...
        for otheratom in self.atomList:
            if ignoreSame == 'res':
                ...
                if otheratom.getAtomID() == atom.getAtomID():
                    continue
            dist = self.getDistanceBetweenAtoms(atom1=atom, atom2=otheratom)
            if dist < distLimMax:
                if dist > distLimMin:
    ...
    def groupAtomsByDist(self,
                         atomType='OH', resType='TYR',
                         resNum='0', chainType='A'):

        # group atoms by their distance
        # away from a specified atom

        atm = self.getAtom(
            chain=chainType, restype=resType,
            resnum=resNum, atomtype=atomType)
        ...
        for othAtm in self.atomList:
            dist = self.getDistanceBetweenAtoms(atom1=keyAtm, atom2=othAtm)
            if dist > maxDist:
                maxDist = dist

        foundAtms = self.getAtomsWithinDist(atom=keyAtm, distLimMax=maxDist+1)
    ...
    def metricValsAtDistFromAtom(self,
                                 atomType='OH', resType='TYR', resNum='0',
                                 chainType='A', metric='loss',
                                 normType='Standard', saveFig=True,
                                 outputDir='./'):
        ...
        atmDic, order = self.groupAtomsByDist(
            atomType=atomType, resType=resType,
            resNum=resNum, chainType=chainType)
        ...
        for stat in ('mean', 'max', 'min'):
            ...
            args = [normType, metric, chainType, resType, resNum,
                    atomType, stat]
            ...
            fileName = self.makeLinePlot(
                vals2Plot=yDic, xVals=xDic, yLabel='D{}'.format(metric),
                title=plotTitle, saveFig=saveFig, fileName=fileName,
                outputDir=outputDir)

    def hotellingTsquareTest(self,
                             atomType1='OH', atomType2='CD',
                             resType1='TYR', resType2='GLU',
                             resNum1='', resNum2='',
                             metric='loss', normType='Standard',
                             atmVals1=[], atmVals2=[], alpha=0.05,
                             printText=True, roundTo=2, roundMVals=8):
        ...
        if atmVals1 == [] or atmVals2 == []:
            ...
            if printText:
                args = [resType1, atmType1, resNum1,
                        resType2, atmType2, resNum2]
                print('Comparing atoms {}-{}-{} and {}-{}-{}'.format(*args))

            # get atoms to be compared
            atoms1 = self.getAtom(restype=resType1, resnum=resNum1,
                                  atomtype=atomType1)

            atoms2 = self.getAtom(restype=resType2, resnum=resNum2,
                                  atomtype=atomType2)
    ...
    def plot_densMetSurroundAtmsCorrel(
        self, restype='TYR', atomtype='OH', distLim=4, errorbars=True,
        normType='Standard', densMet='loss', crystConts=True,
        pdbName='', symmetrygroup='', outputDir='./', fileType='.svg',
            saveFig=True, hotelling=True, printText=True):

        # run .densMetSurroundAtmsCorrel() method for
        # each dataset and plot versus dose

        if errorbars:
            plotDic = {'H-bond': [], 'Other': []}
        else:
            plotDic = {'H-bond': {}, 'Other': {}}

        errBars = {'H-bond': [], 'Other': []}

        for i, d in enumerate(self.getDsetList()):
            output = self.densMetSurroundAtmsCorrel(
                restype=restype, atomtype=atomtype, distLim=distLim,
                dataset=d, normType=normType, densMet=densMet,
                crystConts=crystConts, pdbName=pdbName,
                symmetrygroup=symmetrygroup, plotScatter=True,
                printText=printText, outputDir=outputDir)
        ...
        ylabel = '{} D{}'.format(normType, densMet)
        title = 'Effect of carboxylate H-bonds on {}-{} damage'.format(
            restype, atomtype)
        name = 'carboxylateHbond_vs_{}-{}_damage-normalisation-{}_metric-{}'.format(
            restype, atomtype, normType, densMet)

        fileName = self.makeLinePlot(
            vals2Plot=plotDic, figSize=[12, 8], xVals='doses',
            errors2Plot=errBars, inclErrors=errorbars, yLabel=ylabel,
            title=title, saveFig=saveFig, fileType=fileType,
            fileName=name, outputDir=outputDir, palette='deep')

        if hotelling:
            self.hotellingTsquareTest(atmVals1=HTgroup1, atmVals2=HTgroup2)

        if saveFig:
            return fileName

    def densMetSurroundAtmsCorrel(self,
                                  restype='TYR', atomtype='OH', distLim=4,
                                  dataset='0', normType='Standard',
                                  densMet='loss', crystConts=True,
                                  pdbName='', symmetrygroup='',
                                  fileType='.svg', plotScatter=True,
                                  printText=True, outputDir='./'):
        ...
        if crystConts:
            ...
            ncont = NCONTjob(
                inputPDBfile=pdbName, outputDir=self.outputDir,
                symGroup=symmetrygroup, seriesName=self.seriesName,
                printText=printText)
            success = ncont.run()
            ...
            if crystConts:
                ...
                for l in readLog.readlines():
                    if atomFound and len(l[0:5].strip()) != 0:
                        # reset if new atom info reached
                        atomFound = False
                    ...
                    if atomFound:
                        if (l[39:42] in ('GLU', 'ASP') and
                            ...
                            chain = l[32]
                            if printText:
                                print('contact found! - {}-{}-{}-{}'.format(
                                    chain, resNum, l[39:42], l[47:50]))
                            contactAtom = self.getAtom(
                                chain=chain, restype=l[39:42],
                                resnum=resNum, atomtype=l[47:50])
                            nearCarboxyl = True
                            break
                readLog.close()

            else:
                nearAtms = self.getAtomsWithinDist(
                    atom=atom, distLimMax=distLim)
                nearCarboxyl = False
        ...
        strA = 'For the {} atoms near carboxyl groups, mean avDloss: {}, std: {}'.format(
            len(densValsA), round(np.mean(densValsA), 3), round(np.std(densValsA), 2))
        strB = 'For the {} atoms not near carboxyl groups, mean avDloss: {}, std: {}'.format(
            len(densValsB), round(np.mean(densValsB), 3), round(np.std(densValsB), 2))

        if printText:
            print(strA+'\n'+strB)

        # determine how solvent accessibility changes for each of these atoms
        # upon Glu/Asp decarboxylation. This will dictate the colour scheme
        # for the resulting scatter plot
        scatterColor = []
        solvAccDic, rSqd = self.compareSolvAccessWithAndWithoutGluAspGroups(
            pdbName=pdbName, symmetrygroup=symmetrygroup, dataset=dataset,
            resType=restype, atomType=atomtype, specialAtoms=groupA,
            densMet=densMet, normType=normType, plotScatter=plotScatter,
            printText=printText, outputDir=outputDir)
        ...
        for atm in groupA:
            ...
            if diff == 0:
                if printText:
                    print('{}: No change in solvent accessibility'.format(
                        atmID))
            else:
                if printText:
                    print('{}: Solvent accessibility increased by {} upon ' +
                          'nearby decarboxylation'.format(
                            atmID, round(diff, 2)))
                count += 1
        if count < len(groupA):
            if printText:
                print('Not all {}-{} atoms experience change in solvent ' +
                      'accessibility upon nearby decarboxylation'.format(
                        restype, atomtype))

        if plotScatter:
            # plot the relationship between TYR-OH density
            # and nearby carboxyl atom density
            self.plotScatterPlot(
                xData=densValsA, yData=densValsAContacts,
                ...
                fileType=fileType, colors=scatterColor, outputDir=outputDir)

        return strA+'\n'+strB, densValsA, densValsAContacts, densValsB, scatterColor

    def susceptAtmComparisonBarplot(self,
                                    metric='loss', normType='Standard',
                                    dataset=0, set=1, box='Box', fileType='.svg',
                                    axesFont=18, outputDir='./', saveFig=True):

        # produce a barplot to compare the damage metric of
        # susceptible atom types at a given dataset

        if set == 1:
            keyAtomTypes = [['GLU', 'CD'], ['ASP', 'CG'], ['TYR', 'OH'],
                            ['CYS', 'SG'], ['MET', 'SD'], ['LYS', 'NZ'],
                            ['ARG', 'NE'], ['ASN', 'OD1'], ['GLN', 'OE1'],
                            ['ALA', 'CB']]
        if set == 2:
            keyAtomTypes = [['GLU', 'OE1'], ['GLU', 'OE2'], ['ASP', 'OD1'],
                            ['ASP', 'OD2'], ['TYR', 'OH'], ['ASN', 'OD1'],
                            ['GLN', 'OE1'], ['SER', 'OG'], ['THR', 'OG1'],
                            ['', 'O']]
        ...
        args = [box, normType, metric, dataset, set]
        ...
        if box == 'Box':
            ...
            self.makeBarPlot(plotData=plotData, xName="x", yName="y",
                             xLabel='Atom type', figName=figName,
                             yLabel='{} D{}'.format(normType, metric),
                             axesFont=axesFont, saveFig=saveFig,
                             outputDir=outputDir)
    ...
    def makeBarPlot(self,
                    plotData={}, figSize=[10, 8], color='#0095b6',
                    xName='x-data', yName='y-data', plotType='bar',
                    xLabel='', yLabel='', xRot=90, yRot=0, axesFont=14,
                    figName='', fileType='.svg', saveFig=True, outputDir='./'):
        ...
        if plotType == 'bar':
            ax = sns.barplot(x=xName, y=yName, data=plotData,
                             color=color, saturation=.5)

        elif plotType == 'box':
            ax = sns.boxplot(x=xName, y=yName, data=plotData,
                             color=color, saturation=.5)
        else:
        ...
        fileName = self.savePlot(
            outputDir=outputDir, name=figName, fileType=fileType,
            save=saveFig, f=f)

        if saveFig:
            return fileName

    def compareSolvAccessWithAndWithoutGluAspGroups(
        self, pdbName='', symmetrygroup='', dataset='average', resType='',
        atomType='', specialAtoms='', densMet='loss', normType='Standard',
            fileType='.svg', plotScatter=True, printText=True, outputDir='./'):
        ...
        # run AREAIMOL in CCP4 to calculate solvent
        # accessibility for each pdb file
        count = 0
        for pdb in (pdbName, strippedPdb):
            ...
            commandInput2 = 'DIFFMODE OFF\nMODE -\nNOHOH\nSMODE IMOL\nREPORT -\nCONTACT -\n' +\
                            'YES -\nRESAREA -\nYES\nPNTDEN 10\nPROBE 1.4\nOUTPUT\nEND'
            run = ccp4Job('areaimol', commandInput1, commandInput2,
                          self.outputDir, 'areaimolRun{}.txt'.format(count), '')
        ...
        for atm in TyrOHatms:
            ...
            if resType == 'TYR' and atomType == 'OH':
                for atmtype in ['CG', 'CD1', 'CD2', 'CE1', 'CE2', 'CZ']:
                    TyrRingAtms.append(self.getAtom(
                        chain=atm.chaintype, restype=resType,
                        resnum=atm.residuenum, tomtype=atmtype)[0])
        ...
        for atm in TyrOHatms:
            ...
            args = [key, round(solvAccDic[key][0], 2),
                    round(solvAccDic[key][1], 2), round(plotData['y'][-1], 2)]
        ...
        if plotScatter:
            ...
            # and nearby carboxyl atom density
            rSquared = self.plotScatterPlot(
                xData=plotData['x'], yData=plotData['y'], outputDir=outputDir,
                xLabel='Solvent Accessibility', lineBestFit=True,
                ylabel='{}-{} D{}'.format(resType, atomType, densMet),
                fileType=fileType, colors=plotData['colours'],
                saveTitle='D{}Scatter_{}-{}_vsSolvAccChange_Dset-{}'.format(
                    densMet, resType, atomType, dataset))
    ...
    def calcAverageMetricOverDoses(self,
        ...
        try:
            ...
            self.calcAdditionalMetrics(
                metric=metric, normType=normType, newMetric='average')

    def findProbHighNeighbourGivenHighAtom(self,
                                           distance=5, densMet='loss',
                                           normType='Standard', dataset=0,
                                           criteria='1std', sign='above'):
        ...
        if criteria == 'mean' or 'std' in criteria:
            av, std = self.getAverageMetricVals(
                densMet=densMet, normType=normType)
            if criteria == 'mean':
                thres = av[dataset]
        ...
        for atm in self.atomList:
            ...
            if sign == 'above':
                ...
                return
            numHighAtoms += 1
            nearAtms = self.getAtomsWithinDist(atom=atm, distLimMax=distance)
        ...
        print('Probability of high ({} {}) neighbour atom GIVEN high atom: {}'.format(
            sign, thres, probHighNeighGivenHighAtom))

        return probHighNeighGivenHighAtom

    def calculateLocalDloss(self,
                            resType='', atomType='', distance=4,
                            densMet='loss', normType='Standard',
                            weighted=False, fileType='.svg',
                            printText=True):
        ...
        if len(atms) == 0:
            # don't continue if no atoms exist
            return plotData
        ...
        for atm in atms:
            ...
            if weighted:
                meanDens = np.average(
                    densList, weights=np.square(np.reciprocal(distList)))
            else:
                meanDens = np.mean(densList)

            dens = atm.densMetric[densMet][normType]['average']

            args = [atm.getAtomID(), densMet, dens,
                    densMet,  meanDens]
        ...
        # plot the relationship between atom Dloss and
        # local environment average Dloss
        self.plotScatterPlot(
            xData=plotData['x'], yData=plotData['y'], fileType=fileType,
            xLabel='{}-{} D{}'.format(resType, atomType, densMet),
            ylabel='local environment D{}'.format(densMet),
            figtitle='Average D{} within {} Angstrom of {}-{}'.format(
                densMet, distance, resType, atomType),
            saveTitle='D{}Scatter_{}-{}_localEnvironmentComparison'.format(
                densMet, resType, atomType))

        return plotData

    def getAvMetricPerAtmInRes(self,
                               resType='', densMet='loss', normType='Standard',
                               dataset=0, printText=True):
        ...
        for atm in atms:
            ...
            if atmType not in list(atmDic.keys()):
                atmDic[atmType] = [dens]
            else:
                atmDic[atmType] += [dens]
        for k in list(atmDic.keys()):
